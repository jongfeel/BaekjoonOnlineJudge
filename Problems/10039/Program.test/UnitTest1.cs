...
namespace test
{
    public class CustomDataSourceAttribute : Attribute, ITestDataSource
    {
        ...
        {
            yield return new object[] { new int[] { 10, 65, 100, 30, 95 }, 68};
        }
    ...
    public class UnitTest1
    {
        [DataTestMethod]
        [CustomDataSource]
        public void TestMethod(int[] values, int expected)
        {
            Trace.Listeners.Add(new TextWriterTraceListener(Console.Out));
            Trace.WriteLine($"{values} {expected}");
            Assert.AreEqual(Program.GetSupplementaryAverage(values), expected);
        }
    }
}
